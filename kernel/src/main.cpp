...
// Generate the classic 'knightfetch' ASCII art with live system stats
string get_knightfetch_ascii() {
    stringstream ss;
    ss << "██╗  ██╗███╗   ██╗██╗ ██████╗ ██╗  ██╗████████╗" << endl;
    ss << "██║ ██╔╝████╗  ██║██║██╔════╝ ██║  ██║╚══██╔══╝" << endl;
    ss << "█████╔╝ ██╔██╗ ██║██║██║  ███╗███████║   ██║   " << endl;
    ...
    ss << "OS         : KnightKernel 1.0.0 (user-space)" << endl;
    ss << "Kernel     : C++17 / Node Bridge / React" << endl;
    ss << "Shell      : knightsh 1.0" << endl;
...
int main() {
    ...
    while (getline(cin, line)) {
        ...
        try {
            ...
            // 1. SYSTEM COMMANDS
            if (cmd == "ping") {
                res["msg"] = "KnightKernel Engine 1.0";
            }
            else if (cmd == "knightfetch") {
                res["ascii"] = get_knightfetch_ascii();
            }
            else if (cmd == "exit") {
                cout << json({{"status","ok"},{"msg","Shutting down KnightKernel..."}}).dump() << endl;
                exit(0);
            }
